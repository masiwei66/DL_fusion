...
先对完整数据生成固定的 ID 级划分。正常实验和负控实验必须使用相同的 train/val/test 样本：
...
- **0.3 标签负控**：仅置换固定 split 中 train 的监督；val/test 保留真实标签。默认完整置换多任务监督束，诊断训练只使用材料标签。
...
4. 负控建议至少运行 3 个置换种子。性能应接近训练集 prevalence baseline，而不是要求精确等于某个固定随机数。
5. 阈值必须在查看实验结果前确定。不得通过事后降低过拟合阈值或放宽负控容差来制造“通过”。
6. 阶段 0 的 checkpoint 和指标不得与正式主实验结果混用。
