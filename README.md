...
当前数据集（`data_new`）处于**生成与审计阶段**，审计时仅 11 个结构状态，主要限制：

1. **样本数量不足**：11 个结构状态无法支撑稳定的训练/验证/测试指标，目前只适合代码链路冒烟测试。
2. **类别不平衡**：健康与纯支座场景各仅 1 个，全桥安全/风险状态偏少。
3. **激励多样性不足**：所有样本复用同一条激励力时程，无法评价对未见激励的泛化。
4. **数据域单一**：全部为有限元域，尚未覆盖 FE → 缩尺实验 → 实桥的跨域评估。
5. **静态材料信号较弱**：纯材料削弱对温度位移斜率影响很小，静态分支对单纯构件损伤的敏感性需要通过实验验证。

正式实验前需先补充结构状态、激励种子与目标域数据，再按 `structural_state_id` / `group_id` 做严格划分。

---

## 相关文档

- [RESEARCH_PLAN.md](RESEARCH_PLAN.md) — 研究路线：数据协议、可训练基线、核心 AI 方法、实验矩阵、跨域学习与可信评估。
